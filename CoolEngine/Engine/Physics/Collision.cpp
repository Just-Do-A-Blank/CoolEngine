...
bool Collision::BoxCollision(Box* box1, Box* box2)
{
	...
	XMFLOAT2 middle1 = { box1->m_transform->GetWorldPosition().x, box1->m_transform->GetWorldPosition().y };
	XMFLOAT2 middle2 = { box2->m_transform->GetWorldPosition().x, box2->m_transform->GetWorldPosition().y };
...
{
	XMFLOAT2 pos1 = { circle1->m_transform->GetWorldPosition().x, circle1->m_transform->GetWorldPosition().y };
	XMFLOAT2 pos2 = { circle2->m_transform->GetWorldPosition().x, circle2->m_transform->GetWorldPosition().y };
...
bool Collision::CircleBoxCollision(Circle* circle, Box* box)
{
	...
	XMFLOAT2 middle = { box->m_transform->GetWorldPosition().x, box->m_transform->GetWorldPosition().y };
	...
	XMFLOAT2 distance = { x - circle->m_transform->GetWorldPosition().x , y - circle->m_transform->GetWorldPosition().y };
...
bool Collision::BoxCollisionAndResponse(Box* player, Box* object)
{
	...
	XMFLOAT2 middleP = { player->m_transform->GetWorldPosition().x, player->m_transform->GetWorldPosition().y };
	XMFLOAT2 middleO = { object->m_transform->GetWorldPosition().x, object->m_transform->GetWorldPosition().y };
	...
	if (middleP.x + halfSizeP.x > middleO.x - halfSizeO.x && middleP.x - halfSizeP.x < middleO.x + halfSizeO.x && middleP.y + halfSizeP.y > middleO.y - halfSizeO.y && middleP.y - halfSizeP.y < middleO.y + halfSizeO.y)
	{
		...
		if (abs(penetration.x) < abs(penetration.y))
		{
			...
			{
				// Left side
				XMFLOAT3 pos = { middleO.x - halfSizeO.x - halfSizeP.x, middleP.y, player->m_transform->GetWorldPosition().z };
		...
		else if (abs(penetration.x) > abs(penetration.y))
		{
			...
			{
				// Bottom side
				XMFLOAT3 pos = { middleP.x, middleO.y - halfSizeO.y - halfSizeP.y, player->m_transform->GetWorldPosition().z };
				player->m_transform->SetPosition(pos);
			}
			else
			{
		...
		else
		{
			if (penetration.x > 0)
			{
				...
				{
					// Top right corner
					XMFLOAT3 pos = { middleO.x + halfSizeO.x + halfSizeP.x, middleO.y + halfSizeO.y + halfSizeP.y, player->m_transform->GetWorldPosition().z };
					player->m_transform->SetPosition(pos);
				}
				else
				{
...
bool Collision::CircleBoxCollisionAndResponse(Circle* circle, Box* box)
{
	...
	if (distMagnitude < circle->m_radius)
	{
		...
		{
			XMFLOAT3 pos = { box->m_transform->GetWorldPosition().x + penetration.x, box->m_transform->GetWorldPosition().y + penetration.y, box->m_transform->GetWorldPosition().z };
...
bool Collision::CircleCollisionAndResponse(Circle* circle1, Circle* circle2)
{
	...
	if (m_distanceBetweenCircles <= circle1->m_radius + circle2->m_radius)
	{
		...
		XMFLOAT3 pos = { pos1.x + vectorToPlayer.x / m_distanceBetweenCircles * penetration, pos1.y + vectorToPlayer.y / m_distanceBetweenCircles * penetration, circle1->m_transform->GetWorldPosition().z };
