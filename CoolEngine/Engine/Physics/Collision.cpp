...
{
	XMFLOAT2 halfSize1 = { t1->GetScale().x, t1->GetScale().z };
	XMFLOAT2 halfSize2 = { t2->GetScale().x, t2->GetScale().z };
	XMFLOAT2 middle1 = { t1->GetPosition().x, t1->GetPosition().z };
	XMFLOAT2 middle2 = { t2->GetPosition().x, t2->GetPosition().z };
...
{
	XMFLOAT2 halfSizeP = { player->GetScale().x, player->GetScale().z };
	XMFLOAT2 halfSizeO = { object->GetScale().x, object->GetScale().z };
	XMFLOAT2 middleP = { player->GetPosition().x, player->GetPosition().z };
	XMFLOAT2 middleO = { object->GetPosition().x, object->GetPosition().z };
	...
	if (middleP.x + halfSizeP.x > middleO.x - halfSizeO.x && middleP.x - halfSizeP.x < middleO.x + halfSizeO.x && middleP.y + halfSizeP.y > middleO.y - halfSizeO.y && middleP.y - halfSizeP.y < middleO.y + halfSizeO.y)
	{
		...
		{
			// Up side
			XMFLOAT3 pos = { player->GetPosition().x, player->GetPosition().y, middleO.y + halfSizeO.y + halfSizeP.y };
		...
		else if (vertexToPlayer.x == vertexToPlayer.y)
		{
			...
			{
				// Top right corner
				XMFLOAT3 pos = { middleO.x + halfSizeO.x + halfSizeP.x, player->GetPosition().y, middleO.y + halfSizeO.y + halfSizeP.y };
				player->SetPosition(pos);
			}
